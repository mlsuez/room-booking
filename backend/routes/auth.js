...
router.get('/google', async (req, res) => { try { const url = getGoogleClient().generateAuthUrl({ access_type: 'offline', scope: ['https://www.googleapis.com/auth/calendar.events', 'https://www.googleapis.com/auth/userinfo.email'], prompt: 'consent' }); res.redirect(url); } catch (e) { console.error('Google auth error:', e.message); res.status(500).json({ error: e.message }); } });
router.get('/google/callback', async (req, res) => { try { const client = getGoogleClient(); const { tokens } = await client.getToken(req.query.code); client.setCredentials(tokens); const oauth2 = google.oauth2({ version: 'v2', auth: client }); const { data } = await oauth2.userinfo.get(); req.session.user = { email: data.email, name: data.name || data.email, provider: 'google', tokens }; res.redirect(process.env.FRONTEND_URL); } catch (e) { res.redirect(`${process.env.FRONTEND_URL}?error=google_auth_failed`); } });
let msalClient;
...
router.get('/microsoft', async (req, res) => { try { const url = await getMsalClient().getAuthCodeUrl({ scopes: ['Calendars.ReadWrite', 'User.Read'], redirectUri: `${process.env.BACKEND_URL}/auth/microsoft/callback` }); res.redirect(url); } catch (e) { console.error('Microsoft auth error:', e.message); res.status(500).json({ error: e.message }); } });
